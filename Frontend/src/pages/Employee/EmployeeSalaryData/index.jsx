...
const EmployeeSalaryData = () => {
  ...
  const onSubmitPrint = async () => {
    ...
    try {
      ...
      const nameData = viewSingleEmployeeSalaryByName(employeeName);

      if (yearData.length > 0 && monthData.length > 0 && nameData.length > 0) {
        navigate(`/employee-salary-data/print-page?month=${dataMonth}&year=${dataYear}&name=${employeeName}`);
      } else {
  ...
  useEffect(() => {
    const getEmployeeData = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/salaryData/name/${employeeName}`);
