...
const PrintPdfSalaryReport = () => {
  ...
  const { dataSalarayReport } = useSelector((state) => state.SalarayReport);

  const getDataByYear = async (Year) => {
    dispatch(fetchSalaryReportByYear(Year));
  };

  const getDataByMonth = async (Month) => {
    dispatch(fetchSalaryReportByMonth(Month));
  };

  const handlePrint = useReactToPrint({
    content: () => componentRef.current,
    documentTitle: "Employee_Salary_Data_NFT Autoports Freight Terminal Limited",
  });
      ...
      <div ref={componentRef} className="w-200% h-100% p-10 bg-white dark:bg-meta-4">
        <div className="flex items-center gap-24 object-cover border-b-4 border-black dark:border-white">
          ...
          <h1 className="text-black text-2xl font-bold boder  dark:text-white">
            NFT Autoporst Freight Terminal Limited
          </h1>
          <img className="w-35"
            src={LogoPt}
            title="Logo NFT Autoporst Freight Terminal Limited"
            alt="Logo NFT Autoporst Freight Terminal Limited"
          />
        </div>
        <h1 className="text-center text-black my-4 text-xl font-medium boder py-2 dark:text-white">
          Employee Salart List
        </h1>
        ...
        <div className="max-w-full overflow-x-auto py-4">
          <table className="w-full table-auto-full">
            ...
            <tbody>
              {dataSalarayReport.map((data, index) => {
                return (
                  <tr key={index}>
                    ...
                    <td className="border-b border-l border-black dark:border-white py-5 text-center">
                      <p className="text-black dark:text-white">Rp. {data.basicSalary}</p>
                    </td>
                    <td className="border-b border-l border-black dark:border-white py-5 text-center">
                    ...
                    <td className="border-b border-l border-r border-black dark:border-white py-5 text-center">
                      <p className="text-black dark:text-white">Rp. {data.totalSalary}</p>
                    </td>
                  </tr>
                );
              })}
