...
const FormAddAttendanceData = () => {
    ...
    const saveAttendanceData = async (e) => {
        ...
        try {
            for (let i = 0; i < employeeData.length; i++) {
                ...
                if (!isNameAvailable) {
                    await axios.post("http://localhost:5000/attendanceData", {
                        nid: employeeData[i].nid,
    ...
    return (
        <Layout>
            <Breadcrumb pageName="Form Data employee attendance" />
                        ...
                        <table className="w-full table-auto">
                            ...
                            <tbody>
                                {filteredEmployeeData.slice(startIndex, endIndex).map((data, index) => {
                                    const isNameAvailable = saveAttendanceData.some(
                                        (kepresentan) => kepresentan.employeeName === data.employeeName
                                    );
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <td className="py-5 px-4">
                                                <p className="text-black dark:text-white">{data.nid}</p>
                                            </td>
                                            <td className="py-5 px-4">
                    ...
                    <div className="flex justify-between items-center mt-4 flex-col md:flex-row md:justify-between">
                        <div className="flex items-center space-x-2">
                            <span className="text-gray-5 dark:text-gray-4 text-sm py-4">
                                Displaying {startIndex + 1}-{Math.min(endIndex, filteredEmployeeData.length)} of {filteredEmployeeData.length} Employee Attendance Records
                            </span>
